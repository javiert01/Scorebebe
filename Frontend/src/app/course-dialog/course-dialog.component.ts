...
export class CourseDialogComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<CourseDialogComponent>, @Inject(MAT_DIALOG_DATA) data, private authService: AuthService) {
    ...
    this.parto = data.parto;
    this.apgar = data.apgar;
    this.comor = data.comor;
    this.factorRiesgoInminente = data.factorRiesgoInminente;
  ...
  crearPDF() {
    ...
    const imgHeaderData = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA2YAAABvCAYAAACHMtOrAAAAAXNSR0IArs4' +
      ...
      if (this.factoresRiesgoInminente.length > 0) {
        for (let i = 0; i < this.factoresRiesgoInminente.length; i++) {
          if (this.factoresRiesgoInminente[i] === 'El niño respira débilmente o tiene dificultad respiratoria severa (utilizar la escala de Silverman en las páginas 48 y 49 del AIEPI Clínico)') {
            const aux = 'El niño respira débilmente o tiene dificultad respiratoria severa (utilizar la escala de Silverman en las';
            const aux2 = 'páginas 48 y 49 del AIEPI Clínico)'
...
getCurrentDate() {
  ...
  getCurrentTime() {
    ...
    let time = today.getHours() + 'H' + minutesString;
    return time;
  }
  ...
  daysBetween(date1: string, time: string) {
    ...
    let fechaN = new Date(Number(dateAux[0]), Number(dateAux[1]) - 1, Number(dateAux[2]), Number(timeAux[0]), Number(timeAux[1]));

    // Convert both dates to milliseconds
    const today = new Date();

    let date1_ms = fechaN.getTime();
